...
function removeFirstOccurrences(str, value) {
  const fInd = str.indexOf(value);
  if (fInd !== -1) return str.slice(0, fInd) + str.slice(fInd + value.length);
  return str;
}
...
function findLongestWord(sentence) {
  const bb = sentence.split(' ');
  let maxw = bb[0];
  for (let i = 1; i < bb.length; i += 1) {
    if (maxw.length < bb[i].length) maxw = bb[i];
  }
  return maxw;
}
...
function reverseWords(str) {
  const bb = str.split(' ');
  const revbb = bb.map((qq) => qq.split('').reverse().join(''));
  return revbb.join(' ');
}
